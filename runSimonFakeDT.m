...
[t, y, ~, ~, ~, o] = odeWithTimeout(@dXdT, [0, 22 * params.T], init_vec, options, params, iniGeo, maxTime);
...
if ~isreal(y) || ~isreal(o)
    error('Simulation terminated: y or o contains complex numbers.');
end

if any(y(:) < 0)
    error('Simulation terminated: y contains negative values.');
end

% Check if any of o(5,:) to o(12,:) (pressures) are negative
if any(any(o(5:12,:) < 0))
    error('Simulation terminated: o(5:12,:) contains negative pressure values.');
end
...
function [T, Y, TE, YE, IE, o] = odeWithTimeout(odefun, tspan, y0, options, params, iniGeo, maxTime)
